...
const AddTransaction = () => {
  ...
  const insertTransaction = useMutation({
    mutationFn: (transaction: { amount: number, budget_id: string, label: string }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        const insert = await insertTransaction.mutate(transaction)
        console.log('insert', insert)
        setAmount(0)
        setLabel('')
        setBudgetId('month')
        toast.success('Add transaction successfully', { id: 'addTransaction' })
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="mt-1">
                ...
                />
                <select onChange={(e) => setBudgetId(e.target.value)}>
          ...
          <div>
            <button type="submit" disabled={insertTransaction.isLoading}>
              Add
            </button>
          </div>
        </form>
